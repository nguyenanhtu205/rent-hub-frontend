...
export default function Forbidden() {
  ...
  return (
    <div className='relative flex min-h-screen items-center justify-center overflow-hidden bg-linear-to-br from-sky-50 via-white to-amber-50 px-6 text-slate-900'>
      ...
      <div className='pointer-events-none absolute inset-x-0 top-1/2 h-px bg-linear-to-r from-transparent via-sky-400/40 to-transparent' />

      <div className='relative z-10 mx-auto max-w-xl text-center'>
        {/* 403 with shield lock overlay */}
        <div className='relative mx-auto inline-block'>
          ...
          {/* Floating lock badge over the zero */}
          <div className='pointer-events-none absolute top-1/2 left-1/2 flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-2xl border border-sky-200 bg-white/70 shadow-xl shadow-sky-500/20 backdrop-blur-md sm:h-24 sm:w-24'>
            <Lock className='h-9 w-9 text-sky-700 sm:h-11 sm:w-11' strokeWidth={2.25} />
          </div>
        </div>

        {/* Badge */}
        <div className='mx-auto -mt-4 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-white/80 px-4 py-1.5 text-xs font-medium tracking-widest text-amber-700 uppercase shadow-sm backdrop-blur'>
        ...
        <p className='mx-auto mt-3 max-w-md text-base text-slate-600'>
          Trang tồn tại, nhưng tài khoản của bạn chưa được cấp quyền truy cập. Hãy đăng nhập bằng
          tài khoản phù hợp hoặc liên hệ quản trị viên để được hỗ trợ.
        </p>
        ...
        <p className='mt-12 text-xs text-slate-500'>
          Mã lỗi: <span className='font-mono text-slate-700'>ERR_FORBIDDEN</span>
        </p>
      </div>
    </div>
  );
